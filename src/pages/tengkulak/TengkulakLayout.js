...
const TengkulakLayout = () => {
  ...
  const menuItems = [
    ...
    { key: 'approval', icon: <CheckCircleOutlined />, label: 'Management Approval' },
    { key: 'laporan', icon: <FileTextOutlined />, label: 'Laporan Pembelian' },
    { type: 'divider' },
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      <Sider breakpoint="lg" collapsedWidth="0" onCollapse={(value) => setCollapsed(value)} trigger={null} collapsible collapsed={collapsed} theme="light" style={{ boxShadow: '2px 0 8px 0 rgba(29,35,41,.05)', zIndex: 2 }}>
        <div style={{ height: 32, margin: 16, background: 'rgba(76, 175, 80, 0.1)', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2e7d32', fontWeight: 'bold' }}>
          {collapsed ? 'TK' : 'Panel Tengkulak'}
        </div>
        <Menu theme="light" mode="inline" selectedKeys={[currentPath]} onClick={handleMenuClick} items={menuItems} />
      </Sider>
      <Layout>
        <Header style={{ padding: 0, background: colorBgContainer, display: 'flex', alignItems: 'center', boxShadow: '0 1px 4px rgba(0,21,41,.08)', zIndex: 1 }}>
          ...
          <h2 style={{ margin: 0, fontSize: '18px', color: '#333' }}>Dashboard Tengkulak</h2>
        </Header>
...
export default TengkulakLayout;
