...
const Profile = () => {
  ...
  return (
    <div>
      <Title level={3}>Profil Petani</Title>
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
          <Avatar size={64} icon={<UserOutlined />} style={{ backgroundColor: '#4caf50', marginRight: 16 }} />
          <div>
            <Title level={4} style={{ margin: 0 }}>{user.name || 'Petani'}</Title>
            <p style={{ margin: 0, color: '#666' }}>{user.email || 'petani@manajemenpadi.com'}</p>
          </div>
        </div>
        <Descriptions title="Informasi Akun" bordered column={1}>
          ...
          <Descriptions.Item label="Peran">Petani</Descriptions.Item>
